...
const availabilityHref = "mailto:TrinityClan@proton.me?subject=Tiger%20Gardens%20availability";

export default function InventoryPage() {
  return (
    <main className="tg-static">
      <header className="tg-topbar"><a className="tg-wordmark" href="/"><img src="/tiger-gardens-logo.png" alt="Tiger Gardens" /><span>Tiger Gardens</span></a><nav aria-label="Primary navigation"><a href="/">Home</a><a href="/strains">Cultivars</a><a href="/merch">Merch</a></nav><a className="tg-inventory-link" href="/strains">Cultivar guide <span aria-hidden="true">↗</span></a></header>
      ...
      <section className="tg-inventory-board"><div className="inventory-head"><span>Current garden</span><span>04 cultivars</span></div>{strains.map((cultivar) => <article key={cultivar.id}><span>{cultivar.number}</span><h2>{cultivar.name}</h2><p>In the garden</p></article>)}<a className="static-action" href={availabilityHref}>Ask about availability <span aria-hidden="true">↗</span></a></section>

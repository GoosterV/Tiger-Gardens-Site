...
export default function InventoryPage() {
  return (
    <main className="tg-static">
      ...
      <section className="tg-inventory-board"><div className="inventory-head"><span>Current garden</span><span>03 cultivars</span></div>{inventory.map((cultivar, index) => <article key={cultivar}><span>{String(index + 1).padStart(2, "0")}</span><h2>{cultivar}</h2><p>In the garden</p></article>)}<a className="static-action" href={socialHref} target="_blank" rel="noreferrer">Ask about availability <span aria-hidden="true">↗</span></a></section>
